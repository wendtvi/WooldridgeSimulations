mc_function=function(N){
  ...
  for(p in 1:N){
    ...
    for (k in 1:TT){
      if (k<q) matriz_estado_naotratamento[,k]=as.numeric(-(Z_cov_mean-1)/2-2*D+(Z_cov_mean-1)*D/4+rnorm(n1,0,1)+ct_0>0)
      if (k>=q) matriz_estado_naotratamento[,k]=as.numeric(-(Z_cov_mean-1)/2-2*D+(Z_cov_mean-1)*D/4+rnorm(n1,0,1)+ct_0>0)
    }
    ...
    for (k in 1:ncol(matriz_X_estrela)-1){
      ...
      if (k<q) matriz_X_estrela[,k]=as.numeric(-(Z_cov_mean-1)/2-2*D+(Z_cov_mean-1)*D/4+rnorm(n1,0,1)+ct_inf>0)
